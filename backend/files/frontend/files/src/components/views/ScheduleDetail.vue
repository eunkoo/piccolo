          ...
          <v-card color="grey lighten-4" min-width="350px" flat>
            <v-toolbar :color="editedItem.color" dark>
              <v-btn icon>
                ...
                <v-edit-dialog :return-value.sync="editedItem.name" lazy>
                   <v-icon>mdi-pencil</v-icon>
                  <v-text-field slot="input" v-model="editedItem.name"></v-text-field>
                </v-edit-dialog>
              </v-btn>
...
module.exports = {
  ...
  methods: {
    deleteAttachmentsAll(event_id, schedule_id) {
      ...
      apiService
        .deleteScheduleAttachmentsAll({ eid: event_id, id: schedule_id })
        .then(res => {
    ...
    deleteAttachments(filename) {
      ...
      apiService
        .deleteScheduleAttachments({
          eid: this.editedItem.eid,
    ...
    uploadAttachments() {
      if (this.selectedFile) {
        console.log(
          ` try upload attachments. eid:${this.editedItem.eid}  id:${this.editedItem.id} file:${this.selectedFile.name}`
        );
        ...
        apiService
          .uploadScheduleAttachments(formData)
          .then(res => {
    ...
    mouseCellEntered({ date }) {
      ...
      if (
        +new Date(date).setHours(0, 0, 0, 0) >=
          +new Date(this.start_day).setHours(0, 0, 0, 0) &&
        +new Date(date).setHours(0, 0, 0, 0) <=
          +new Date(this.end_day).setHours(0, 0, 0, 0)
      )
        event.target.style.backgroundColor = "rgba(128, 255, 128, 0.3)";
      else event.target.style.backgroundColor = "rgba(255, 128, 128, 0.3)";
    },
    ...
    async getSchedules(items, current) {
      ...
      await apiService
        .fetchScheduleForEvent({ eid: current.id })
        .then(res => {
      ...
      for (let index = 0; index < this.schedules.length; index += 1) {
        ...
        // console.log(
        //   `   try get attachments list. eid:${item.eid} id:${item.id}`
        // );
        await apiService
          .fetchScheduleAttachments({ eid: item.eid, id: item.id })
          .then(res => {
    ...
    newEvent(e) {
      ...
      const open = () => {
        ...
        this.selectedElement =
          event != undefined ? event.target : this.$refs.calendar; //nativeEvent.target
    ...
    saveEvent() {
      ...
      if (!this.isEdit) {
        ...
        console.log(
          `  request add new schedule. event:${this.editedItem.eid} schedule:${this.editedItem.id}`
        );
        apiService
          .addSchedule(
          ...
          .then(res => {
            ...
            console.log(res.data);
            console.log(
              ` editedItem eid: ${this.editedItem.eid} id:${this.editedItem.id} `
            );
        ...
        console.log(
          `  request edit schedule. event:${this.editedItem.eid} schedule:${this.editedItem.id}`
        );
        apiService
    ...
    removeEvent() {
      ...
      this.selectedOpen = false;
      apiService
        .deleteSchedule(this.editedItem.id)
        .then(res => {
          this.editedItem.resetContent;
        })
        .catch(e => {

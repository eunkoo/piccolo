      ...
      <v-flex xs3>
        ...
        >
          ...
          <v-date-picker 
            v-model="start_date" 
            :show-current="true"
            @input="$refs.menuBeginDate.save(start_date)"></v-date-picker>
        </v-menu>
      </v-flex>
    <v-flex xs3>
      <v-menu
        ...
        >
          ...
          <v-date-picker 
            v-model="end_date" 
            :show-current="true"
            @input="$refs.menuEndDate.save(end_date)"></v-date-picker>
        </v-menu>
      </v-flex>
...
export default {
  ...
  data () {
    return {       
      ...
      // start_date: '',
      // end_date: '',
      start_date: this.initDate().toISOString().substr(0,10),
  ...
  methods:{
    ...
    async fillChartData(){
      ...
      const alldate = vm.getDaysBetweenDates(new Date(vm.start_date), new Date(vm.end_date), "Sun");
      vm.zzchartdata = [];
      for ( let index = 0; index < vm.category.length; index += 1) {

        // console.log(`통계 데이터 요청. ${vm.category[index].text} index:${index}`);
        let res = await apiService.fetchGraph(index, vm.start_date, vm.end_date);

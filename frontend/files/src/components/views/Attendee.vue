<template>
    ...
    ></v-progress-circular>
        ...
        <v-flex  :key="5" xs3 sm3 md3 v-if='userGrade==0'>
           <v-btn fab color="primary" @click="save">저장
           </v-btn>
        ...
        <v-layout v-if='!printing '    justify-center align-center >
            <v-flex :key="6" xs4  >
            <v-btn label="prev" small fab @click="onPrev" ><v-icon large color="black">chevron_left</v-icon></v-btn >
            </v-flex>
                <v-flex xs4 >
            ...
            <v-flex :key="7" xs4  >
            <v-btn label="next" fab small @click="onNext" > <v-icon large  color="black">chevron_right</v-icon></v-btn >
            </v-flex>
        </v-layout>
             <v-flex v-if="!printing && userGrade==0">
                 <!-- <v-container class="guide_layout">
                     <v-layout class="guide"> -->
                        <event-detail ref='detail' :printing="true" @changeOffering='changeOffering' v-bind:datas="editedItem"></event-detail>
                     <!-- </v-layout>
                 </v-container> -->
                  <editable-attendee ref='editable' :attendee="attendee" @changeNoon='changeNoonAtt' @changeMorning='changeMorningAtt'> </editable-attendee>
             </v-flex>
             <v-flex v-else>
            ...
            <div  id="printMe" class="page-wrapper"  >
                <div class="page" >
                    <div class="page-inner">
                        <v-flex >
                        <twin-table :isHeader="true"  @changeNoon='changeNoonAtt' @changeMorning='changeMorningAtt' :tableinfo="table_info_text" :attendee1="attendee1" :attendee2="attendee2"></twin-table>
                        <twin-table :isHeader="false"  @changeNoon='changeNoonEx' @changeMorning='changeMorningEx' :tableinfo="'새신자 및 장기 결석'" :attendee1="expectee1" :attendee2="expectee2"></twin-table>
              ...
              <event-notice v-if="!printing && userGrade==0" @changeNotice='changeNotice' v-bind:printingview="false" ref='notice' v-bind:datas="editedItem"></event-notice>
         <!-- <v-layout class="guide"  justify-center align-center >
...
export default {
  name: 'attendee',
  components:{
    Datepicker,EventDetail,EventNotice ,TwinTable, downloadExcel, EditableAttendee
  },
  data () {
    return {
        events:[],
  ...
  methods: {
    formatDate(date) {
      ...
      async addAttendee(){
        ...
        if(item.length === 0){
            ...
            var ndata = Object.assign( tmp , {"id":'', "eid":this.editedItem.id, "mid":-1, "name":this.addAttendName,"grade":0,"connected":this.addAttendConnected,"bible":0,"paragraph":false, "attended":0})
            try{
            await apiService.addAttendee( ndata )
            .then((result) => {
            ...
            try{
                var satt = item[0]
                await apiService.deleteAttendee( satt.id )
                .then((result) => {
                    console.log( result.data)
      ...
      setInfoText( belong){
          console.log('setInfoText '+belong)
           console.log(this.belong_items[belong].text)
            this.table_info_text=  this.cdateFormat +' '+this.belong_items[belong].text+' '+'출결 - 재적 : '
            +(this.editedItem.totalmember==undefined ? 0 : this.editedItem.totalmember)+', 오전 : '
            +(this.editedItem.totalmorning==undefined? 0 : this.editedItem.totalmorning) +", 오후 : "
            +(this.editedItem.totalnoon==undefined ?   0 : this.editedItem.totalnoon)
      },
      changeNotice(notice){
    ...
    async getevents (belong) {
      ...
      if(this.events.length===0) {
            this.editedItem = {"day":this.cdate,"belongs":belong,"title":this.ctitle,"place":this.cplace}
            this.attendee = []
            return
      }
    ...
    async saveNames(belong){
        ...
        await this.names.forEach(item => {
            var ndata = {"eid":this.editedItem.id, "mid":item.id, "name":item.name,"grade":item.grade,"connected":item.connected, "bible":0,"paragraph":false, "attended":0, "comment":""}
    ...
    loadEventData(belong, cdate){
        ...
        if(item.length >0) {
            ...
            this.editedItem ={"day":this.cdate.toDateString(),"belongs":this.cbelong,"title":this.ctitle,"place":this.cplace}
            this.editedIndex =-1
        }
        this.cbelong = this.editedItem .belongs
        this.ctitle = this.editedItem .title;
    ...
    async getAttendee(belong,date){
        ...
        if(this.attendee.length == 0 ){ // to be created
